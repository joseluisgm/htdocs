...
class Wow_List_Table extends WP_List_Table {
	...
	public function column_title( $item ) {
		...
		$actions       = array(
			...
			'duplicate' => '<a href="' . esc_url($duplicate_url) . '" class="has-text-success">' . esc_attr__( 'Duplicate', $text )
			               . '</a>',
			'delete'    => '<a href="' . esc_url( wp_nonce_url($delete_url, $slug . '_nonce' ) ) . '" class="has-text-danger">' . esc_attr__( 'Delete', $text ) . '</a>',
		);
	...
	private function table_data() {
		...
		if ( $result ) {
			foreach ( $result as $key => $value ) {
				...
				$status_on      = '<a href="'.esc_url( $deactivate_url).'" class="button is-info is-small has-tooltip-bottom" data-tooltip="' . esc_attr( $tooltip_off ) . '">' . esc_attr__( 'Activated', 'modal-window' ) . '</a>';
				$status_off     = '<a href="'.esc_url( $activate_url).'" class="button is-danger is-small has-tooltip-bottom" data-tooltip="' . esc_attr( $tooltip_on ) . '">' . esc_attr__( 'Deactivated', 'modal-window' ) . '</a>';
	...
	public function get_bulk_actions() {
		$actions = array(
			...
			'activate-items'   => esc_attr__( 'Activate popups', 'modal-window' ),
			'deactivate-items' => esc_attr__( 'Deactivate popups', 'modal-window' ),
		);
